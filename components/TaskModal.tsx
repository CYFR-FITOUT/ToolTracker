...
const ToolModal: React.FC<ToolModalProps> = ({ isOpen, onClose, onSave, toolToEdit }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-70 flex justify-center items-center z-50 transition-opacity" onClick={onClose}>
      <div className="bg-gray-800 rounded-lg p-8 w-full max-w-md m-4 shadow-2xl" onClick={e => e.stopPropagation()}>
        <h2 className="text-2xl font-bold mb-6 text-white">{toolToEdit ? 'Edit Tool' : 'Create New Tool'}</h2>
        <form onSubmit={handleSubmit}>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
            <div className="col-span-2 sm:col-span-1">
              <label htmlFor="name" className="block text-sm font-medium text-gray-300 mb-1">Tool Name</label>
              <input
                id="name"
                type="text"
                value={name}
             ...
             <div className="col-span-2 sm:col-span-1">
              <label htmlFor="inventoryCode" className="block text-sm font-medium text-gray-300 mb-1">Inventory Code</label>
              <input
                id="inventoryCode"
                type="text"
                value={inventoryCode}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
            <div>
              <label htmlFor="currentHolder" className="block text-sm font-medium text-gray-300 mb-1">Assigned To (Holder)</label>
             ...
             <div>
              <label htmlFor="currentLocation" className="block text-sm font-medium text-gray-300 mb-1">Location</label>
          ...
          <div className="flex justify-end gap-4">
            <button type="button" onClick={onClose} className="px-4 py-2 bg-gray-600 hover:bg-gray-500 text-white font-semibold rounded-md transition">Cancel</button>
            <button type="submit" className="px-4 py-2 bg-yellow-600 hover:bg-yellow-700 text-white font-semibold rounded-md transition">Save</button>
          </div>
        </form>
      </div>
    </div>
